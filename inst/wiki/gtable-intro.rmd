...
require(devtools)
require(knitr)
opts_chunk$set(fig.width=3,fig.height=3, tidy=FALSE, cache=TRUE)
```
...
* [based on `grid.arrange`](https://gist.github.com/baptiste/6652815)

```{r gtable_arrange}
source_gist(6652815, quiet=TRUE, sha1='29f6750feb69972e0c74b4d825125308a926329e')
gs <- lapply(1:9, function(ii) grobTree(rectGrob(gp=gpar(fill=ii, alpha=0.5)),
                                        textGrob(ii)))
gtable_arrange(ncol=4, grobs=gs, 
               top="top\nlabel", bottom="bottom\nlabel", 
               left="left\nlabel", right="right\nlabel")
grid.rect(gp=gpar(fill=NA))
```
* [based on a pre-defined layout](https://gist.github.com/baptiste/7070429)
```{r gtable_layout}
source_gist(7070429, quiet=TRUE, sha1='fbde6b1479778e4043cb94b22e3a7eb4bb2fee6c')
gt <- gtable_layout(gs, m=rbind(c(1,1,1,2,3),
                                c(1,1,1,4,5),
                                c(6,7,8,9,9)))
grid.draw(gt)
grid.rect(gp=gpar(fill=NA))
